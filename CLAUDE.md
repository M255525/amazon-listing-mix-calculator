...
**`cost`（商品成本，台幣）刻意避免公式化**：第一版曾統一用「售價換算台幣後的 45%~55%」估算 30 項商品的成本，使用者實測後指出這樣「看起來還是售價、不是真的成本」——比例太集中在同一個窄區間，數字明顯是算出來的而非真實各自獨立的進貨成本。修正版改成每項商品各自指定一個落在 22%~68% 之間、依商品特性合理但不套公式的成本比例（例如電子零件類/戶外機能布料成本占比較高落在 55%~68%，木製砧板/寵物砂這類原料簡單的成本占比較低落在 26%~31%），確保跟售價之間沒有肉眼可辨的固定比例關係。已重新用 Playwright 核對 5 組範例的「總庫存成本」KPI（居家124,350／3C配件26,305／露營245,000／美妝80,300／寵物49,665），並確認沒有商品的成本意外超過換算後售價（會誤觸發 `ruleBasedDiagnosis()` 的「成本已超過售價」警示）。

...
**2026-09-11 第三輪：「商品成本」欄位整個撤掉，改成售價的唯讀台幣換算**。使用者實測後兩度反饋這個欄位讓人困惑：第一次說「商品成本（NT$）是建議售價\*匯率」，我以為是嫌範例資料的成本比例太公式化（45%~55%窄區間），改成 22%~68%更分散的比例；使用者再次反饋「商品成本（NT$）是售價換算過來，應該是建議售價(NT$)」，並在被追問澄清時直接打斷、給出明確指令：「在商品清單，欄位名稱是建議售價，乘\*匯率後，欄位名稱應該是"建議售價(NT$)"，請取代原名稱"商品成本($)"」。**結論：使用者從頭到尾要的其實不是一個獨立的成本追蹤欄位，而是想在「建議售價」旁邊直接看到換算成台幣的參考數字**；「再增加一個成本欄位」這個最初的需求被這次指令實質撤銷。因此把整個 `cost` 概念（商品資料欄位、輸入框、KPI卡、PDF/Word報告列、CSV欄位、規則式健檢的成本警示句）全部移除，商品表格裡原本的「商品成本（NT$）」欄位改成唯讀的「建議售價(NT$)」欄位，內容是 `price × rateForCurrency(priceCurrency)`，跟著售價/幣別/匯率即時連動更新（見下方 `syncPriceTwdCells()`）。下面各節提到「商品成本」「cost」「總庫存成本」的地方已依此更新為現況；未更新的舊敘述以本節為準。
...
- `state` — `{weeklyThreshold, monthlyThreshold, restockLeadDays, capEnabled, capAmount, usdToTwd, jpyToTwd, categories:[{id,name,feePct}], products:[{id,name,categoryId,price,priceCurrency,stock,weeklySales,monthlySales,weeklyProfit,monthlyProfit}], _catSeq, _prodSeq}`，整包存 `localStorage` key `amazonListingMixState`。分類/商品刪除用事件委派（`tbody` 上綁一個 `input`/`change`/`click` listener，用 `closest('tr').dataset.id` 找目標），只有新增/刪除列或切換範例時才整段 `renderCategories()`/`renderProducts()` 重建 DOM，單純編輯欄位值不重建（避免輸入中失焦）。`price` 固定以 `priceCurrency`（`"USD"`／`"JPY"`，預設 `USD`）輸入。**沒有 `cost` 欄位**——2026-09-11 第三輪需求已整個移除，見上方「這是什麼」一節。
- `calculate()` — 核心計算源，回傳 `{perProduct, totals, catRollup}`。`perProduct[i].weeklyNet/monthlyNet` = 毛利 ×（1－該分類抽成%），**不受幣別/匯率影響**；`daysLeft` = 庫存 ÷（週銷量÷7），週銷量為 0 時為 `null`（不計入補貨警示，避免除以零）；`priceTWD` = `price × rateForCurrency(priceCurrency)`（`rateForCurrency()` 依幣別回傳 `state.usdToTwd` 或 `state.jpyToTwd`）；`listingValue` = `priceTWD × 庫存`（售價未填則為 `null`，不計入 `totals.listingTotal`）。`classifyProductRoles()` 的售價百分位判斷改用 `priceTWD` 而非原始 `price`，避免 USD 與 JPY 數字直接混比。
- `syncPriceTwdCells()` — 商品表格「建議售價(NT$)」欄位是唯讀 `<td data-role="priceTwd">`（不是 `<input>`），不走一般的「編輯欄位值不重建 DOM」路徑；改成每次 `recalcAll()` 執行完都跑這個函式，依 `tr.dataset.id` 找回對應商品，重新算 `price × rateForCurrency(priceCurrency)` 寫回該儲存格文字。這樣使用者編輯售價、切換幣別、或改全域匯率，這個唯讀欄位都會即時跟著變、不需要整段重建商品表格（也不會害其他正在輸入中的欄位失焦）。
...
2026-09-11 第二輪追加需求後，5 組範例的售價全部重新設計成貼近真實的 USD／JPY 零售價格（例如電動打蛋器 $24.99、露營帳篷 $89.99、面膜組 ¥1480），而非把原本無幣別的 NT$ 數字直接貼上幣別標籤（那樣會出現「$899的打蛋器」這種失真情境）；`weeklyProfit`/`monthlyProfit`（毛利，跟幣別無關）完全不動，所以收支平衡/補貨警示的既有驗證結果不受影響，只有跟「售價」相關的上架金額/上限比對/角色分類售價百分位需要重新驗證。

**`cost`（商品成本）已於第三輪需求整個移除**（見上方「這是什麼」一節的完整經過），30 項商品曾經各自帶過的成本數字（先是統一 45%~55%、後改 22%~68%）已不存在，範例資料現在只有 `price`/`priceCurrency`，商品表格用 `price × rateForCurrency()` 即時算出「建議售價(NT$)」顯示。

以上數字已用 claude-in-chrome（`javascript_tool`，port 8814 本機預覽伺服器）逐一點開五組範例並讀 `#kpiGrid`/`#restockTbody` 文字內容核對，跟手算結果完全吻合（含分類混合抽成、庫存可撐天數、上限比對、USD/JPY匯率換算）；新增/刪除分類與商品、刪除「有商品使用中」的分類會被擋下、手動編輯商品的售價/幣別即時更新「建議售價(NT$)」與KPI（含全域匯率變更即時連動，未重建整段商品表格）、CSV匯出/匯入不再含成本欄位、PDF與Word報告內容一致且不再提及成本、規則式健檢不再有成本警示句、手機寬度(375px)與桌機寬度(1411px)皆已驗證無橫向溢出，也一併驗證過。

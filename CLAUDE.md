...
**2026-09-11 第四輪：毛利改自動計算＋建議售價(NT$)從唯讀改回可編輯（雙欄互補模式）**。使用者接著提兩點：(1)「在商品清單中，毛利會自己計算」；(2)「在建議售價與建議售價(NT$)只要有一個欄位輸入就可以，但是如果自己輸入建議售價就會換算建議售價(NT$)」。第(1)點單獨問過使用者要哪種公式（毛利=售價(NT$)×銷量 vs 週/月毛利互推），使用者選了前者，並確認代價是「毛利」在本工具裡實質上變成「抽成前營收額」（因為已無成本欄位可扣）——**這個「抽成前」的認知後來在第五輪被使用者糾正為錯誤，見下**。第(2)點則直接說明了「建議售價(NT$)」不該再是純唯讀顯示，而要能雙向當輸入用：
...
**2026-09-11 第五輪：毛利公式算錯了，抽成要扣在毛利這一層，不是淨利那一層；新增「淨利係數」概念**。使用者發現第四輪的公式有誤：「如果amazon抽15%，毛利應該是85%，如果建議售價(NT$)100，月銷量60,毛利應該是5100，但現在是6000」——第四輪把 `weeklyProfit`/`monthlyProfit`（表格裡標「毛利」的那兩欄）設成純營收（`priceTWD×銷量`，範例裡是100×60=6000），完全沒扣 Amazon 抽成，抽成% 只用在「淨利」這一層（`weeklyNet=weeklyProfit×(1-feePct/100)`）。使用者指出這樣不對：「毛利」這個詞在他的認知裡本來就該是「已經扣掉平台抽成後」的數字（100×60×0.85=5100）。同時使用者也指出結果儀表板的「總月淨利（抽成後）」這個標籤/計算方式有問題——「抽成之後應該還是毛利」，要求在全域設定新增一個係數（要我建議名稱），變成「總月毛利×係數＝總月淨利」。

**結論與實作**：把「抽成扣除」從淨利那一層搬到毛利那一層，並在毛利跟淨利之間插入一個新的全域係數：

- `calculate()` 裡 `weeklyProfit`（毛利）＝`priceTWD×週銷量×(1-該分類feePct/100)`（`monthlyProfit` 同理，用月銷量）——**抽成％現在扣在這裡**，跟商品所屬分類的抽成一一對應，逐商品各自扣自己分類的抽成後加總，不是先加總再扣一個籠統的抽成。
- 新增 `state.netCoefficient`（我建議命名**「淨利係數」**，因為使用者原句就是「毛利 × 係數 = 淨利」這個結構，直接呼應；預設 `100`，也就是 100%——**刻意選這個預設值**，因為它讓 `weeklyNet=weeklyProfit×(100/100)=weeklyProfit`，數值上跟原本（第四輪之前）已驗證過的 `weeklyNet` 公式完全等價（`priceTWD×銷量×(1-fee%)`），代表 5 組範例的門檻/上限比較結果**完全不需要重新調校**，這輪唯一數值上會變的是「毛利」本身的顯示數字（現在會比營收小，乘了 `(1-fee%)`），「淨利」數字維持不變）。`calculate()` 裡 `weeklyNet = weeklyProfit × (state.netCoefficient/100)`。
- 全域設定新增「淨利係數（%，毛利 × 此比例＝淨利）」輸入框，UI 說明文字把它定位成「概括 Amazon 抽成以外、本工具沒有逐項列出的其他營運費用（廣告、退貨、倉儲、人事等）」的概括係數，使用者可以自己往下調低來模擬這些沒被拆項的成本。
- `syncComputedCells()`（商品表格「毛利（週/月，自動）」唯讀欄位）也要知道每個商品所屬分類的 `feePct` 才能算對，改成先建 `catMap` 再查表——這是本輪唯一需要碰 DOM 同步函式的地方，因為淨利係數本身不會顯示在逐商品列，只影響彙總層級的 KPI/報告。
- KPI 新增「總週毛利（已扣抽成）」「總月毛利（已扣抽成）」兩張卡，放在原本「總週/月淨利」卡片前面，直接呈現「毛利 → ×淨利係數 → 淨利」這條算式；淨利卡片標籤改成「總週/月淨利（毛利×淨利係數）」，不再用容易被誤讀成「抽成之後」的舊標籤「（抽成後）」。PDF／Word／AI診斷prompt 的整體收支概況同步補上毛利兩行與淨利係數一行。
- `downloadProductsCsv()` 原本獨立算了一份 `weeklyProfit=twd×週銷量`（沒扣抽成），是这次順便發現並修掉的第二個同類 bug——現在改成跟 `calculate()` 一致的公式（也要查分類 `feePct`）。
- 已用使用者原句的數字（建議售價(NT$)100、月銷量60、抽成15% → 毛利5100）＋claude-in-chrome 逐一核對 5 組範例 KPI（確認淨利數字跟第四輪完全一致、毛利數字改變）＋手動把淨利係數改成 70% 驗證淨利正確等比例縮小、毛利不受影響，三項都驗證通過。

下面各節（架構／週邊功能／CSV）已依這五輪的最終狀態更新；若哪裡還留著「唯讀」「weeklyProfit 是輸入欄位」「毛利=純營收」「總月淨利（抽成後）」之類的舊敘述，以本節為準。
...
- `state` — `{weeklyThreshold, monthlyThreshold, restockLeadDays, capEnabled, capAmount, usdToTwd, jpyToTwd, netCoefficient, categories:[{id,name,feePct}], products:[{id,name,categoryId,price,priceCurrency,priceTWD,stock,weeklySales,monthlySales}], _catSeq, _prodSeq}`（`netCoefficient` 為第五輪新增，預設 `100`，見下方「第五輪」），整包存 `localStorage` key `amazonListingMixState`。分類/商品刪除用事件委派（`tbody` 上綁一個 `input`/`change`/`click` listener，用 `closest('tr').dataset.id` 找目標），只有新增/刪除列或切換範例時才整段 `renderCategories()`/`renderProducts()` 重建 DOM，單純編輯欄位值不重建（避免輸入中失焦，`usdToTwd`/`jpyToTwd` 匯率輸入例外，見下）。`price` 固定以 `priceCurrency`（`"USD"`／`"JPY"`，預設 `USD`）輸入；`priceTWD` 可以是 `price × 匯率` 自動算出、也可以是使用者直接填的手動值，見上方「第四輪」說明。**沒有 `cost` 欄位、也沒有 `weeklyProfit`/`monthlyProfit` 儲存欄位**——第三輪移除了成本，第四輪把毛利也改成純衍生值，不再存進 state。
- `calculate()` — 核心計算源，回傳 `{perProduct, totals, catRollup}`。`perProduct[i].priceTWD` 直接取用 `p.priceTWD`（不再在這裡乘匯率，匯率換算已經在資料輸入端的 `recomputePriceTwd()` 做完）；`weeklyProfit`（毛利）＝`(priceTWD!=null?priceTWD×weeklySales:0) × (1-該分類feePct/100)`，`monthlyProfit` 同理（**抽成扣在毛利這一層，見下方「第五輪」**，不是先算純營收）；`weeklyNet/monthlyNet`（淨利）＝`weeklyProfit/monthlyProfit × (state.netCoefficient/100)`；`daysLeft` = 庫存 ÷（週銷量÷7），週銷量為 0 時為 `null`（不計入補貨警示，避免除以零）；`listingValue` = `priceTWD × 庫存`（`priceTWD` 未填則為 `null`，不計入 `totals.listingTotal`，且**不扣抽成**——上架金額是庫存估值不是銷售額）。`classifyProductRoles()` 的售價百分位判斷用 `priceTWD`，避免 USD 與 JPY 數字直接混比。
...
- **一次性 OAuth 授權已完成並端對端驗證通過**（2026-09-11）：`clasp deploy` 本身只是透過 API 建立部署記錄，不會自動觸發那個「Google 尚未驗證這個應用程式」的同意畫面，這一步只能由使用者親自用瀏覽器打開 exec 網址點過「進階→前往（不安全）→允許」（`google-apps-script` skill 明載的限制，Claude 不能代勞）。使用者完成後，已用 claude-in-chrome 實際跑過一次完整流程驗證：(1) 在共用 Sheet 的「AmazonListingMix序號」分頁手動加一列測試序號 `TEST2026`；(2) 在 `index.html` 的序號登入遮罩輸入該序號送出，畫面正確顯示「🔑 剩餘 365 天」徽章、遮罩消失；(3) 回頭檢查 Sheet，該列「開始日期」「結束日期」已被後端自動寫入（2026/9/11 → 2027/9/11，正好 12 個月），證明 `Code.gs` 的開卡邏輯正確運作。**踩坑記錄**：用 `curl -sL`／`curl --post302` 測 `doPost` 兩次都失敗（分別回傳 411 Length Required 與 Google Drive「找不到網頁」錯誤頁），這是 Apps Script exec 網址對匿名 curl POST 請求的已知轉址怪癖，不代表後端壞掉——`doGet` 回傳正確 JSON 就已經足以證明 OAuth 授權生效，真正的 POST 驗證邏輯必須靠瀏覽器實際走一次登入流程才測得出來，不能單靠 curl 判斷。**另一個踩坑**：用 claude-in-chrome 直接在 Google Sheets 網頁介面點按座標編輯儲存格時，多次直接對著儲存格 `type` 中文字（「序號」）都沒有真正寫入內容（疑似 Sheets 的 canvas 格線對自動化 keystroke 的中文輸入不穩定）；改用「`javascript_tool` 呼叫 `navigator.clipboard.writeText()` 寫入剪貼簿 → 點選目標儲存格 → `ctrl+v` 貼上」才成功寫入，之後若需要用瀏覽器自動化編輯 Google Sheets 儲存格內的中文內容，優先用這個剪貼簿貼上手法，不要對著儲存格直接打中文。**⚠️ 目前 Sheet 裡還留著這組測試序號 `TEST2026`（有效期至 2027/9/11）**，正式提供序號給學員/客戶前記得清掉或另外處理。
